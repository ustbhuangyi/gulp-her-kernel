...
function File(file) {
  ...
  //nomarlize path
  this.path = her.util.normalize(this.path);
  ...
  //get file id
  var id = her.util.relative(her.project.getPath(), this.path).replace(/[\/\\]+/g, '/');
...
File.prototype.addSameNameRequire = function (ext) {
  var path;
  var file = this.pathNoExt + ext;
  if (her.util.isFile(file)) {
    path = './' + her.util.basename(file, ext) + ext;
  } else {
    var map = her.config.get('roadmap.ext');
    for (var key in map) {
      if (map.hasOwnProperty(key)) {
        var oExt = normalizeExt(key);
        var rExt = normalizeExt(map[key]);
        file = this.pathNoExt + oExt;
        if (rExt === ext && her.util.isFile(file)) {
          path = './' + her.util.basename(file, oExt) + oExt;
          break;
        }
      }
    }
  ...
  if (path) {
    var info = her.uri.getId(path, this.dirname);
...
function getReleasePath(path, file) {
  path = her.util.normalize(path + '/' + file.relative);
  var rExt = file.rExt;
  var qRExt = her.util.pregQuote(rExt);
  var qExt = her.util.pregQuote(file.ext);
  var reg = new RegExp(qRExt + '$|' + qExt + '$', 'i');
  return path.replace(reg, '') + rExt;
}
